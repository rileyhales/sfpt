...
let ERFP_MANAGE_WATERSHEDS = (function() {
    ...
    *************************************************************************/
     ...
     *************************************************************************/
    let getModalHTML, initializeModal, initializeTableFunctions, getTablePage;
    ...
    initializeModal = function() {
        ...
        $('#edit_modal_submit').off().click(function () {
           ...
           //clear message div
            ...
            let watershed_id = $("#watershed_id").val();
            let watershed_name = checkInputWithError($('#watershed-name-input'),safe_to_submit);
            let subbasin_name = checkInputWithError($('#subbasin-name-input'),safe_to_submit);
            let data_store_id = checkInputWithError($('#data-store-select'),safe_to_submit, true);
            let geoserver_id = checkInputWithError($('#geoserver-select'),safe_to_submit, true);

            //initialize values
            let ecmwf_data_store_watershed_name = "";
            let ecmwf_data_store_subbasin_name = "";
            let geoserver_drainage_line_layer = "";
            let geoserver_boundary_layer = "";
            let geoserver_gage_layer = "";
            let geoserver_historical_flood_map_layer = "";
            let geoserver_ahps_station_layer = "";
            let drainage_line_shp_files = [];
            let boundary_shp_files = [];
            let gage_shp_files = [];
            let ahps_station_shp_files = [];

            //Initialize Data Store Data
            //check ecmwf inputs
            let ecmwf_ready = false
            ...
            if (safe_to_submit.val && !m_uploading_data) {
                if (window.confirm("Are you sure? You will delete prediction files " +
                    ...
                    let xhr = null;
                    let xhr_no_files = null;
                    let xhr_boundary = null;
                    let xhr_gage = null;
                    let xhr_ahps_station = null;
                    let xhr_ecmwf_rapid = null;
                    ...
                    if ($('#shp-upload-toggle').bootstrapSwitch('state')) {
                        ...
                        if (drainage_line_shp_files.length >= 4 ||
                            ...
                            for (let i = 0; i < drainage_line_shp_files.length; i++) {
                                data.append("drainage_line_shp_file", drainage_line_shp_files[i]);
                            }
                            let drainage_success_message = "Drainage Line Upload Success!";
                            if (drainage_line_shp_files.length >= 4) {
                            ...
                            jQuery.when(xhr).done(function (return_data) {
                                ...
                                if (boundary_shp_files.length >= 4) {
                                    ...
                                    data.append("geoserver_historical_flood_map_layer", geoserver_historical_flood_map_layer);
                                    data.append("geoserver_ahps_station_layer", geoserver_ahps_station_layer);
                                    for (let i = 0; i < boundary_shp_files.length; i++) {
                                ...
                                jQuery.when(xhr_boundary).done(function (boundary_data) {
                                    ...
                                    if (gage_shp_files.length >= 4) {
                                        ...
                                        data.append("geoserver_historical_flood_map_layer", geoserver_historical_flood_map_layer);
                                        data.append("geoserver_ahps_station_layer", geoserver_ahps_station_layer);
                                        for (let i = 0; i < gage_shp_files.length; i++) {
                                    ...
                                    jQuery.when(xhr_gage).done(function(gage_data){
                                        ...
                                        if (ahps_station_shp_files.length >= 4) {
                                            ...
                                            data.append("geoserver_historical_flood_map_layer", geoserver_historical_flood_map_layer);
                                            data.append("geoserver_ahps_station_layer", geoserver_ahps_station_layer);
                                            for (let i = 0; i < ahps_station_shp_files.length; i++) {
                        ...
                        let xhr_no_files = ajax_update_database("submit", data);
    ...
    initializeTableFunctions = function() {
        ...
        $('#watershed_table').on("click", ".submit-delete-watershed", function () {
            ...
            //update database
            let xhr = deleteRowData($(this), data, "main_message");
            if (xhr != null) {
