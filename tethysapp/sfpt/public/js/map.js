...
let ERFP_MAP = (function() {
     ...
     *************************************************************************/
    let public_interface,                // Object returned by the module
        m_map,                    // the main map
     ...
     *************************************************************************/
    let stringToUTCmiliseconds, stringToUTCDate, resizeAppContent, bindInputs,
    ...
    stringToUTCmiliseconds = function(datetime_string) {
        let start_year = parseInt(datetime_string.substring(0,4));
        let start_month = parseInt(datetime_string.substring(4,6));
        let start_day = parseInt(datetime_string.substring(6,8));
        let start_hour = 0;
        let string_split = datetime_string.split(".");
    ...
    convertTimeSeriesEnglishToMetric = function(time_series, series_name) {
        let series_data = [];
        let series_dates = [];
        let date_time_value, data_value;
        let conversion_factor = 1;
        if (m_units == "metric") {
            conversion_factor = 35.3146667;
        }
    ...
    getCI = function(obj, prop){
        ...
        for(let key in obj.getProperties()){
    ...
    getValidSeries = function(series_array){
        if (series_array != null) {
            ...
            for (let i=0; i<series_array.length; i++) {
                valid_series = true;
                for (let j=0; j<series_array[i].length; j++) {
    ...
    zoomToFeature = function(watershed_info, reach_id) {
        if(!m_searching_for_reach) {
            ...
            let search_id_button = $("#submit-search-reach-id");
            let search_id_button_html = search_id_button.html();
            search_id_button.text('Searching ...');
            let watershed_split = watershed_info.split(":");
            let watershed_name = watershed_split[0];
            let subbasin_name = watershed_split[1];
            ...
            m_drainage_line_layers.forEach(function(drainage_line_layer, j) {
                if(drainage_line_layer.get('watershed_name') == watershed_name &&
                    ...
                    if (drainage_line_layer.get('layer_type') == "geoserver") {
                        ...
                        let reach_id_attr_name = getCI(drainage_line_layer, 'reach_id_attr_name');
                        if (reach_id_attr_name != null) {
                            //TODO: Make query more robust
                            let url = drainage_line_layer.get('geoserver_url') +
    ...
    getTileLayer = function(layer_info, geoserver_url, layer_id, visible, input_opacity) {
        //validate extent
        let extent = layer_info['latlon_bbox'].map(Number)
    ...
    getDrainageLineLayer = function(watershed_layers_info) {
        let drainage_line_layer_id = 'layer-' + watershed_layers_info.id + '-drainage_line';
        if ("error" in watershed_layers_info.drainage_line) {
            ...
            if(watershed_layers_info.drainage_line.geoserver_method == "natur_flow_query") {
                let load_features_xhr = null;
                let drainage_line_vector_source = new ol.source.Vector({
                    format: new ol.format.GeoJSON(),
                    url: function(extent, resolution, projection) {
                        let stream_flow_limit = 5000;
                        let map_zoom = m_map.getView().getZoom();
                        if (map_zoom >= 12) {
                            stream_flow_limit = 0;
                        } else if (map_zoom >= 11) {
    ...
    updateDownloadForecastURL = function() {
        ...
        let sel_data = $('#download-select').select2('data');
        if (sel_data != null)
        {
    ...
    addSeriesToCharts = function(series_data){
        ...
        let existing_chart = $("#long-term-chart .js-plotly-plot");
    ...
    createEmptyForecastChart = function(){
        ...
        let y_axis_title = "Streamflow (m<sup>3</sup>/s)";
        if (m_units == "english") {
    ...
    getChartData = function(from_units_toggle) {
        ...
        if(!isNotLoadingPastRequest()) {
                ...
                .done(function(response) {
                        ...
                        for (let object1 in data) {
                            if (object1 == "dates") {
                                ...
                                let cellcolor = "purple"
                            }
                            if (object1 == "dates") {
                                let tr = "<tr id=" + object1.toString() + "><th>Dates</th>";
                                for (let value1 in data[object1]) {
                                    tr += "<th>" + data[object1][value1].toString() + "</th>"
                                }
                                tr += "</tr>";
                                tbody.innerHTML += tr;
                            } else {
                                let tr = "<tr id=" + object1.toString() + "><td>" + columNames[object1.toString()] + "</td>";
                                for (let value1 in data[object1]) {
            ...
            let date_observed_end =  date_now;
            let date_nws_end = date_future;

            //ECMWF Dates
            let ecmwf_date_forecast_begin = new Date(8640000000000000);
            let ecmwf_date_forecast_end = new Date(-8640000000000000);
            let get_ecmwf = m_ecmwf_forecast_folder != null && typeof m_ecmwf_forecast_folder != "undefined" &&
                m_ecmwf_forecast_folder != "most_recent";
            //get ECMWF forcast dates if available
            if(get_ecmwf) {
                let ecmwf_forecast_start_year = parseInt(m_ecmwf_forecast_folder.substring(0,4));
                let ecmwf_forecast_start_month = parseInt(m_ecmwf_forecast_folder.substring(4,6));
                let ecmwf_forecast_start_day = parseInt(m_ecmwf_forecast_folder.substring(6,8));
                let ecmwf_forecast_start_hour = parseInt(m_ecmwf_forecast_folder.split(".")[1].substring(0,2));
            ...
            let date_observed_start = new Date(Math.min.apply(null,[date_past, ecmwf_date_forecast_begin]));
            let date_nws_start = new Date(Math.min.apply(null,[date_now, ecmwf_date_forecast_begin]));
                ...
                if(m_selected_usgs_id.length >= 8) {
                    ...
                    .done(function (data) {
                        if (typeof data != 'undefined') {
                            try {
                                let time_series_data = convertTimeSeriesEnglishToMetric(data.value.timeSeries[0].values[0].value, "USGS");
                                let usgs_series = [{
            ...
            if(m_selected_nws_id != null) {
                ...
                .done(function(data) {
                    //let series_data = getValidSeries(WATERML.get_json_from_streamflow_waterml(data, m_units));
                    let series_data = WATERML.get_json_from_streamflow_waterml(data, m_units, "T0 (Time of analysis)");
                    if(series_data == null) {
            ...
            if(m_selected_hydroserver_url != null) {
                ...
                .done(function(data) {
                    let series_data = WATERML.get_json_from_streamflow_waterml(data, m_units);
                    if(series_data == null) {
                        appendErrorMessage("No data found for WorldWater", "hydro_server_error", "message-error");
                    } else {
                        let hydro_server_series = [{
                            name: "HydroServer",
                            x: series_data[0].dates,
                            y: series_data[0].data,
                            line: {
    ...
    loadHydrographFromFeature = function(selected_feature, from_units_toggle) {
        ...
        if(!isNotLoadingPastRequest()) {
            ...
            //get attributes
            let reach_id = getCI(selected_feature, 'COMID');
            let ecmwf_watershed_name = getCI(selected_feature, "watershed");
            let ecmwf_subbasin_name = getCI(selected_feature, "subbasin");
            let usgs_id = getCI(selected_feature, "usgs_id");
            let nws_id = getCI(selected_feature, "nws_id");
            let hydroserver_url = getCI(selected_feature, "hydroserve");
            
            //check if the variables are under a different name
            if(reach_id == null || isNaN(reach_id)) {
                let reach_id = getCI(selected_feature, 'hydroid');
            }
    
            if(ecmwf_watershed_name == null) {
                let ecmwf_watershed_name = getCI(selected_feature, 'watershed_name');
            }
            if(ecmwf_subbasin_name == null) {
                let ecmwf_subbasin_name = getCI(selected_feature, 'subbasin_name');
            }
    ...
    loadWarningPoints = function(watershed_layer_group, group_id, datetime_string) {
        ...
        let xhr2 = xhr.done(function (data) {
            let feature_array = (new ol.format.GeoJSON()).readFeatures(data, {featureProjection: 'EPSG:3857'});
            if (feature_array.length > 0) {
                $(group_id).parent().removeClass('hidden');
                let first_layer = null;
                let feature_dict = {};
                watershed_layer_group.getLayers().forEach(function(sublayer, j) {
                    let peak_date = stringToUTCDate(datetime_string);
    ...
    updateWarningPoints = function(datetime_string) {
        ...
        let warning_xhr_list = [];
        m_map.getLayers().forEach(function(watershed_layer, i){
            if (watershed_layer.get('layer_type') == "warning_points") {
                if (watershed_layer instanceof ol.layer.Group) {
                    let group_id = '#'+watershed_layer.get('group_id');
                    let loaded = false;
                    watershed_layer.getLayers().forEach(function(sublayer, j) {
                        if (sublayer instanceof ol.layer.Group) {
    ...
    updateWarningSlider = function(datetime_string) {
        ...
        let dateSlider = document.getElementById('warning-date-slider-range');
        if (dateSlider != null) {
            let noui_data = dateSlider.noUiSlider;
            ...
            dateSlider.noUiSlider.on('update', function( values, handle ) {
                ...
                let warning_date_start = new Date();
                warning_date_start.setTime(+values[0]);
                let warning_date_end = new Date();
                            ...
                            layer.getLayers().forEach(function(sublayer, j) {
                                if (sublayer instanceof ol.layer.Group) {
                                    if (sublayer.get("daily_warnings")) {
                                        sublayer.getLayers().forEach(function(subsublayer, j) {
                                            let layer_date = subsublayer.get('peak_date');
                                    ...
                                    let layer_date = sublayer.get('peak_date');
                                    if(typeof layer_date != 'undefined') {
                                        sublayer.setVisible(true);
                                        if (layer_date < warning_date_start || layer_date > warning_date_end) 
                                        {
    ...
    *************************************************************************/
    ...
    $(function() {
        ...
        let twenty_symbols = [new ol.style.RegularShape({
                                              points: 3,
                                              radius: 9,
                                              fill: new ol.style.Fill({
        ...
        //symbols
        let ten_symbols = [new ol.style.RegularShape({
                                              points: 3,
                                              radius: 9,
                                              fill: new ol.style.Fill({
        ...
        //symbols
        let two_symbols = [new ol.style.RegularShape({
                                              points: 3,
                                              radius: 9,
                                              fill: new ol.style.Fill({
        ...
        //load base layer
        let base_layer_info = JSON.parse($("#map").attr('base-layer-info'));
        let warning_point_forecast_folder = $("#map").attr('warning-point-start-folder');
        m_basemap_layer = getBaseLayer(base_layer_info.name);
        
        //load drainage line layers
        let watershed_layers_info_array = JSON.parse($("#map").attr('watershed-layers-info-array'));
        let watershed_group_info_array = JSON.parse($("#map").attr('watershed-group-info-array'));

        let all_watershed_layers = [];
        m_drainage_line_layers = [];
        m_predicted_flood_maps = [];

        let watershed_group_iteration_array = [];
        let divide_into_groups = false;
        ...
        watershed_group_iteration_array.forEach(function(watershed_group_info, group_iteration_index) {
            ...
            let group_drainage_line_layers = [];
            let group_boundary_layers = [];
            let group_gage_layers = [];
            let group_historical_flood_map_layers = [];
            let group_ahps_station_layers = [];
            let group_return_20_layers = [];
            let group_return_10_layers = [];
            let group_return_2_layers = [];
            ...
            watershed_group_info.watershed_layers_info.forEach(function(watershed_layers_info) {
                ...
                if('boundary' in watershed_layers_info) {
                    let boundary_layer_id = 'layer-' + watershed_layers_info.id + '-boundary';
                    if ("error" in watershed_layers_info.boundary) {
                        ...
                        let boundary_layer = getTileLayer(watershed_layers_info.boundary,
                ...
                if('gage' in watershed_layers_info) {
                    let gage_layer_id = 'layer-' + watershed_layers_info.id + '-gage';
                    if ("error" in watershed_layers_info.gage) {
                        ...
                        let gage_layer = getTileLayer(watershed_layers_info.gage,
                                                      watershed_layers_info.geoserver_url, 
                                                      gage_layer_id, 
                                                      divide_into_groups, 
                                                      0.5);
                ...
                if('historical_flood_map' in watershed_layers_info) {
                    let historical_flood_map_layer_id = 'layer-' + watershed_layers_info.id + '-historical_flood_map';
                    if ("error" in watershed_layers_info.historical_flood_map) {
                        ...
                        let historical_flood_map_layer = getTileLayer(watershed_layers_info.historical_flood_map,
                                                                      watershed_layers_info.geoserver_url, 
                    ...
                    if ('geoserver_info_list' in watershed_layers_info.predicted_flood_maps) {
                        ...
                        watershed_layers_info.predicted_flood_maps.geoserver_info_list.forEach(function(flood_map_info, flood_map_index){
                            ...
                            if ("error" in flood_map_info) {
                                ...
                                let layer = getTileLayer(flood_map_info,
                                                         watershed_layers_info.geoserver_url, 
                ...
                if('ahps_station' in watershed_layers_info) {
                    let ahps_station_layer_id = 'layer-' + watershed_layers_info.id + '-ahps_station';
                    if ("error" in watershed_layers_info.ahps_station) {
                        ...
                        let ahps_station_vector_source = new ol.source.Vector({
                            format: new ol.format.GeoJSON(),
                            url: function(extent, resolution, projection) {
                        ...
                        let ahps_station = new ol.layer.Vector({
                ...
                if('drainage_line' in watershed_layers_info) {
                    let drainage_line = getDrainageLineLayer(watershed_layers_info);
                ...
                let return_20_layer = getWarningPointsLayerGroup('rgba(128,0,128,0.7)',
                                                                 twenty_symbols, 
                                                                 20, 
                ...
                let return_10_layer = getWarningPointsLayerGroup('rgba(255,0,0,0.6)',
                                                                ten_symbols, 
                                                                10, 
                ...
                let return_2_layer = getWarningPointsLayerGroup('rgba(255,255,0,0.3)',
                                                               two_symbols, 
                                                               2, 
            ...
            if(divide_into_groups) {
                let drainage_line_layer = determineGeoServerLayerOrGroup(group_drainage_line_layers, divide_into_groups,
                ...
                let boundary_layer = determineGeoServerLayerOrGroup(group_boundary_layers,
                                                                    divide_into_groups, 
                ...
                let gage_layer = determineGeoServerLayerOrGroup(group_gage_layers,
                                                                divide_into_groups, 
                ...
                let historical_flood_map_layer = determineGeoServerLayerOrGroup(group_historical_flood_map_layers,
                                                                                divide_into_groups, 
                ...
                let ahps_station_layer = determineGeoServerLayerOrGroup(group_ahps_station_layers,
                                                                        divide_into_groups, 
                ...
                let warnings_2_year = determineGeoServerLayerOrGroup(group_return_2_layers,
                                                                     divide_into_groups,
                ...
                let warnings_10_year = determineGeoServerLayerOrGroup(group_return_10_layers,
                                                                      divide_into_groups, 
                ...
                let warnings_20_year = determineGeoServerLayerOrGroup(group_return_20_layers,
                                                                      divide_into_groups,
        ...
        let all_map_layers = [m_basemap_layer].concat(all_watershed_layers);
        ...
        all_watershed_layers.forEach(function(watershed_layer){
            let group_id = '#'+watershed_layer.get('group_id');
            let layer_id = '#'+watershed_layer.get('layer_id');
            if (watershed_layer instanceof ol.layer.Group) {
                if (watershed_layer.get('layer_type') == "warning_points") {
                    let loaded = false;
                    watershed_layer.getLayers().forEach(function(sublayer, j) {
                    ...
                    bindInputs(group_id, watershed_layer);
                    let group_extent = ol.extent.createEmpty();
                    watershed_layer.getLayers().forEach(function(sublayer, j) {
        ...
        $('.flood_map_select').off().change(function() {
            let watershed_name = $(this).parent().parent().parent().parent().attr('watershed');
            let subbasin_name = $(this).parent().parent().parent().parent().attr('subbasin');
            let date_timestep = $(this).val();
        ...
        $('.zoom-to-layer').off().click(function() {
            let layer_id = $(this).parent().parent().attr('id');
            zoomToLayer(layer_id);
        });

        //function to zoom to feature by id
        $('#submit-search-reach-id').off().click(function() {
            let watershed_info = $('#watershed_select').select2('val');
            let reach_id = $('#reach-id-input').val();
        ...
        $('#warning_point_date_select').on('change.select2', function () {
            let forecast_folder = $(this).select2('data').id;
